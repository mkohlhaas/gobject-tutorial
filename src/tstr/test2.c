...
notify_cb (GObject *gobject, GParamSpec *pspec, gpointer user_data)
{
  ...
  if (strcmp (name, "string") == 0 && strcmp ((s = t_str_get_string (T_STR (gobject))), "hello") == 0)
    {
      notify_signal = 1;
    }
  if (s)
...
main (void)
{
  ...
  char *t_name[] = { "t_none", "t_int", "t_double" };
  struct str_type
  {
    char *s;
    int   type;
  } sample[] = { { "123", t_int },         { "-45", t_int },         { "+0", t_int },      { "123.456", t_double },
                 { "+123.456", t_double }, { "-123.456", t_double }, { ".456", t_double }, { "123.", t_double },
                 { "0.0", t_double },      { "abc", t_none } };
      ...
      if (strcmp (s, sample[i].s) != 0 || (a = t_num_str_get_string_type (numstr)) != sample[i].type)
        {
          if (t_none <= a && a <= t_double)
            {
              g_print ("g_object_set/get didn't work. %s was expected, but actual returned value was %s and the type "
                       "was %s.\n",
