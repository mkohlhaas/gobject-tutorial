...
t_print (const char *cmp, TComparable *c1, TComparable *c2)
{
  ...
  TStr *ts1 = t_str_new_with_string ("\"");
  if (T_IS_NUMBER (c1))
    {
...
main (void)
{
  ...
  TInt    *i    = t_int_new_with_value (124);
  TDouble *d    = t_double_new_with_value (123.45);
  TStr    *str1 = t_str_new_with_string (one);
  TStr    *str2 = t_str_new_with_string (two);
  TStr    *str3 = t_str_new_with_string (three);
